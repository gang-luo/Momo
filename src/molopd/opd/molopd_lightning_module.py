...
class MolOPDLightningModule(pl.LightningModule):
    ...
    def __init__(self, cfg: dict[str, Any] | Any):
        ...
        out_dir = cfg.get("runtime", {}).get("output_dir", cfg.get("logging", {}).get("output_dir", "outputs/molopd"))
    ...
    def configure_optimizers(self):
        optim_cfg = self.cfg.get("optimizer", self.cfg.get("training", {}))
        params = [p for p in self.student.parameters() if p.requires_grad]
        if not params:
            raise RuntimeError("No trainable student parameters found for MolOPD.")
        ...
        if not sched_cfg or str(sched_cfg.get("name", "none")).lower() in {"", "none"}:
            return opt
        sched_name = str(sched_cfg.get("name", "cosine")).lower()
        if sched_name == "cosine":
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                opt,
                T_max=int(sched_cfg.get("t_max", 1000)),
                eta_min=float(sched_cfg.get("eta_min", 1e-6)),
            )
        elif sched_name == "multistep":
            scheduler = torch.optim.lr_scheduler.MultiStepLR(
                opt,
                milestones=list(sched_cfg.get("milestones", [1000, 2000])),
    ...
    def on_save_checkpoint(self, checkpoint: dict[str, Any]) -> None:
        ...
        save_dir = f"{self.cfg.get('runtime', {}).get('output_dir', self.cfg.get('logging', {}).get('output_dir', 'outputs/molopd'))}/{self.cfg.get('logging', {}).get('run_name', 'run')}/student_step_{int(self.global_step)}"
